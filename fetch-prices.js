...
- 
- Reads prices.json (creates if missing), appends new points, writes back.
- Keeps last 30,000 points per pair (~100 days at 5min intervals).
  */
...
// Use median of all prices to avoid outlier ads
const prices = d.data
...
// ── FILTRO ANTI-OUTLIER ──────────────────────────
// Descarta precios anomalos (anuncios mal puestos en Binance)
// Si la desviacion del promedio reciente es >5%, ignora el dato
function isOutlier(price, history) {
if (history.length < 5) return false; // muy pocos datos para comparar
